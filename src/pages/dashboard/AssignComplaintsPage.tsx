...
import { UserCheck, Eye, Pencil, Trash2 } from 'lucide-react';
...
const AssignComplaintsPage = () => {
  const [complaints, setComplaints] = useState(
    initialComplaints.filter(c => c.status === 'Categorized' || c.status === 'New')
  );
                  ...
                  <TableRow key={c.id} className="hover:bg-muted/50">
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1">
                        ...
                        <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-muted-foreground" title="Edit" onClick={() => { setSelected(c); setSheetOpen(true); }}>
                          <Pencil className="w-4 h-4" />
                        </Button>
                        <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-destructive" title="Delete" onClick={() => setDeleteTarget(c)}>
                          <Trash2 className="w-4 h-4" />
                        </Button>
                        <Button size="sm" className="text-xs gradient-primary text-primary-foreground h-8 px-2">Assign</Button>
                      </div>
                    </TableCell>
                  </TableRow>
                )) : (

...
const ApprovalsPage = () => {
  const [responses, setResponses] = useState<Response[]>(
    initialResponses.filter(r => r.response_status === 'Pending Approval' || r.response_status === 'Draft')
  );
  ...
  const handleApprove = (r: Response) => {
    handleUpdate({ ...r, response_status: 'Approved', approved_by: 'Team Lead A', approved_date: new Date().toISOString().split('T')[0] });
  };
  const handleReject = (r: Response) => {
    handleUpdate({ ...r, response_status: 'Draft' });
  };
            ...
            <Table>
              ...
              <TableBody>
                {responses.length > 0 ? responses.map(r => (
                  <TableRow key={r.id} className="hover:bg-muted/50">
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1">
                        <Button size="sm" className="text-xs gradient-primary text-primary-foreground h-7 px-2" onClick={() => handleApprove(r)}>Approve</Button>
                        <Button size="sm" variant="outline" className="text-xs text-destructive h-7 px-2" onClick={() => handleReject(r)}>Reject</Button>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-1">
                  ...
                  <TableRow>
                    <TableCell colSpan={9} className="text-center text-muted-foreground py-8">No responses pending approval</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
